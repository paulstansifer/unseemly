...
fn main() {
    ...
    if arguments.len() == 1 {
        ...
        terminal_display(eval_unseemly_program_top(&raw_input));
    } else if arguments.len() == 3 {
        let (pc, type_env, type_env__phaseless, value_env) =
            language_from_file(&std::path::Path::new(&arguments[1]));
...
pub fn html_render(res: Result<Value, String>) -> String {
    match res {
        Ok(v) => format!("<b>{}</b>", v),

        ...
        let res = match *self.rule {
            ...
            Seq(_) | Star(_) | Plus(_) | SynImport(_, _, _) => {
                ...
                Ok(self.locate(done_tok, match *self.rule {
                    ...
                    Star(_) | Plus(_) => ast::IncompleteNode(if subtrees.len() > 0 {
                        EnvMBE::new_from_anon_repeat(
                            subtrees.into_iter().map(|a| a.flatten()).collect(),
                        )
                    } else {
                        let depth_is_one = |(n, depth)| {
                            assert_eq!(depth, 1, "how is {} repeated?", n);
                            n
                        };
                        EnvMBE::new_from_empty_anon_repeat(
                            &self.rule.binders().into_iter().map(depth_is_one).collect::<Vec<_>>(),
                        )
                    }),

    ...
    Rc::new(Form {
        ...
        synth_type: if pos {
            ::form::Positive(cust_rc_box!(|quote_parts| {
                if nt_is_positive(vr_to_name(&quote_parts.get_term(n("nt")))) {
                    ...
                    let _ = &quote_parts
                        .with_context(prot_expected_type)
                        .get_res(n("body"));
        ...
        eval: if pos {
            ...
            Negative(cust_rc_box!(|quote_parts| {
                ...
                let mq_parts = quote_parts
                    .switch_mode::<QQuoteDestr>()
                    .quote_more(None)
...
fn quote_unquote_type_basic() {
    ...
    assert_eq!(
        ...
        Ok(
            ty!({"Type" "type_apply" : "type_rator" => (,pat_type.clone()),
            "arg" => [{"Type" "Nat" :}]})
        )
    );
    ...
    assert_eq!(
        ...
        Ok(
            ty!({"Type" "type_apply" : "type_rator" => (,expr_type.clone()),
            "arg" => [{"Type" "struct":
                "component_name" => [@"c" "x", "y", "z"],
